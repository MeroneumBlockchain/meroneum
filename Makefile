...
build-docker-goleveldb:
	...
	mkdir -p ./build/.meroneumd
	echo '#!/usr/bin/env bash' > ./build/meroneumd
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/meroneumd
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/meroneumd
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.meroneumd:/home/evmos/.meroneumd $$IMAGE_NAME meroneumd "$$@"' >> ./build/meroneumd
	chmod +x ./build/meroneumd
